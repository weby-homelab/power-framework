...
| **Гібридний пошук (v2.0)**      | 5-режимний пошук: FTS5 (BM25), Dense Vector Semantic, Hybrid (RRF), Semantic та Hybrid Reranked. Бекенд за замовчуванням — **`Qwen3-Embedding-0.6B`** (1024d, ONNX, мультилінгвальний, рекомендовано для UA↔EN vault) або `fastembed` MiniLM-L12 (384d, легкий). Включає розширення запитів (синоніми/LLM) та контекстне чанкування (`SemanticChunker`) |
| **Cross-Encoder реранкер**      | Режим `hybrid_reranked` використовує мультилінгвальний **`Qwen3-Reranker-0.6B-ONNX`** (через `qwen3-embed`, без PyTorch). Виправляє стару поведінку MiniLM-реранкера, що погіршувала мікс-мовну якість (MAR@5 −22%, ×8 latency)                                                                                                                         |
...
flowchart TD
    ...
    subgraph RAG ["🔍 Конвеєр RAG & GraphRAG"]
        ...
        Embeddings["🧠 Dense ембеддінги<br/>(Qwen3-0.6B def / MiniLM)"]:::rag
        SQLite[("🗄️ SQLite (FTS5 + chunk_embeddings)")]:::rag
        Expander["🔄 Розширювач запитів (Synonyms / LLM)"]:::rag
        Reranker["🎯 Cross-Encoder реранкер (Qwen3-0.6B)"]:::rag
...
| `core/embeddings.py`      | Підключний менеджер ембеддінгів: **Qwen3-0.6B (def, 1024d)** / MiniLM-L12-v2 (384d, light) через `POWER_EMBED_PROVIDER`, ONNX Runtime, lazy init, adaptive batch halving на OOM |
| `core/reranker.py`        | Cross-Encoder реранкер: **Qwen3-Reranker-0.6B-ONNX** (def при provider=qwen3, без PyTorch) або `ms-marco-MiniLM-L-6-v2` fallback                                                |
