...
# P.O.W.E.R. 3.7.7 — AI-Native Toolkit для Second Brain
...
- **Реліз P.O.W.E.R. 3.7.7** — публікація вимагає перевірені
  wheel, source archive, SBOM, Linux upgrade matrix і fresh release receipts.
  Версія 3.7.7 містить canonical Task v2 і typed decision workflows,
...
- **[Матриця підтримки платформ](docs/support-matrix.ua.md)** — Linux є
  release-платформою `v3.7.7`; CI зараз перевіряє її на Ubuntu, а macOS і
...
> **Контракт релізу `v3.7.7`:** використовуйте signed tag, wheel, source
> archive, SBOM і release receipt лише після їх появи на
> [сторінці релізу GitHub](https://github.com/weby-homelab/power-framework/releases/tag/v3.7.7).
...
`v3.7.7`. Окремий [гід Windows 11 25H2](docs/windows-11-installation.ua.md)
описує установку та host validation, але не є сертифікацією Stable release для
цієї платформи.

Для `v3.7.7` немає Windows CI, upgrade-matrix, compatibility, performance або
...
> **Контракт evidence для POWER 3.7.7:** публікація вимагає machine validation
...
historical evidence, а не поточним контрактом `v3.7.7`:
...
URL wheel нижче є tag-bound install target `v3.7.7`. Запускайте його лише
після появи signed tag та assets на
[сторінці релізу](https://github.com/weby-homelab/power-framework/releases/tag/v3.7.7)
