...
- **Реліз P.O.W.E.R. 3.6.3** — публікація вимагає перевірені
  wheel, source archive, SBOM, Linux upgrade matrix і fresh release receipts.
  Версія 3.6.3 додає canonical Task v2 і typed decision workflows,
...
- **[Матриця підтримки платформ](docs/support-matrix.ua.md)** — Linux є
  release-платформою `v3.6.3`; CI зараз перевіряє її на Ubuntu, а macOS і
...
> **Контракт релізу `v3.6.3`:** використовуйте signed tag, wheel, source
> archive, SBOM і release receipt лише після їх появи на
> [сторінці релізу GitHub](https://github.com/weby-homelab/power-framework/releases/tag/v3.6.3).
...
`v3.6.3`. Окремий [гід Windows 11 25H2](docs/windows-11-installation.ua.md)
описує установку та host validation, але не є сертифікацією Stable release для
цієї платформи.

Для `v3.6.3` немає Windows CI, upgrade-matrix, compatibility, performance або
...
> **Контракт evidence для POWER 3.6.3:** публікація вимагає machine validation
...
historical evidence, а не поточним контрактом `v3.6.3`:
...
URL wheel нижче є tag-bound install target `v3.6.3`. Запускайте його лише
після появи signed tag та assets на
[сторінці релізу](https://github.com/weby-homelab/power-framework/releases/tag/v3.6.3)
...
- [Нотатки релізу P.O.W.E.R. 3.6.3](docs/release-3.6.3.md) — Canonical Task v2, typed decisions, fail-closed persistence, reranked search і межі evidence.
