...
- **P.O.W.E.R. 3.6.5 candidate** — publication requires a verified
  wheel, source archive, SBOM, Linux upgrade matrix, and fresh release
  receipts. Version 3.6.5 carries canonical Task v2 and typed decision workflows,
...
- **[Platform support matrix](docs/support-matrix.md)** — Linux is the
  `v3.6.5` release platform; CI currently validates it on Ubuntu, while macOS
...
> **`v3.6.5` candidate release contract:** use the signed tag, wheel, source archive,
> SBOM, and release receipt only after they appear on the
> [GitHub release page](https://github.com/weby-homelab/power-framework/releases/tag/v3.6.5).
...
```bash
python3 -m pip install https://github.com/weby-homelab/power-framework/releases/download/v3.6.5/power_framework-3.6.5-py3-none-any.whl
...
Windows 11 25H2 remains outside the `v3.6.5` supported-platform boundary. The
...
made for `v3.6.5`. Windows and macOS have no scheduled release target.
...
> **POWER 3.6.5 evidence contract:** publication requires machine validation
...
and MCP inventory are historical evidence, not the current `v3.6.5` contract:
...
The wheel URL below is the tag-bound `v3.6.5` install target. Run it only after
the signed tag and assets exist on the
[release page](https://github.com/weby-homelab/power-framework/releases/tag/v3.6.5)
with the source archive, SBOM, and release receipts.

```bash
pip install https://github.com/weby-homelab/power-framework/releases/download/v3.6.5/power_framework-3.6.5-py3-none-any.whl
```
...
MACHINE-READABLE-METADATA: JSON-LD BELOW
{
  ...
  "operatingSystem": "Linux (v3.6.5 release boundary)",
