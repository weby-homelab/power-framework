...
- **P.O.W.E.R. 3.7.1 candidate** — publication requires a verified
  wheel, source archive, SBOM, Linux upgrade matrix, and fresh release
  receipts. Version 3.7.1 carries canonical Task v2 and typed decision workflows,
...
- **[Platform support matrix](docs/support-matrix.md)** — Linux is the
  `v3.7.1` release platform; CI currently validates it on Ubuntu, while macOS
...
> **`v3.7.1` candidate release contract:** use the signed tag, wheel, source archive,
> SBOM, and release receipt only after they appear on the
> [GitHub release page](https://github.com/weby-homelab/power-framework/releases/tag/v3.7.1).
...
Windows 11 25H2 remains outside the `v3.7.1` supported-platform boundary. The
...
made for `v3.7.1`. Windows and macOS have no scheduled release target.
...
> **POWER 3.7.1 evidence contract:** publication requires machine validation
...
and MCP inventory are historical evidence, not the current `v3.7.1` contract:
...
The wheel URL below is the tag-bound `v3.7.1` install target. Run it only after
the signed tag and assets exist on the
[release page](https://github.com/weby-homelab/power-framework/releases/tag/v3.7.1)
