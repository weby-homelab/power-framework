...
- **[CLI reference](docs/cli.md)** — all 27 commands, flags, and actual exit behavior
- **[MCP server](docs/mcp-server.md)** — all 21 governed tools, rate limits,
...
- **[Platform support matrix](docs/support-matrix.md)** — Linux is the
  `v3.7.11` release platform; CI currently validates it on Ubuntu, while macOS
...
> **`v3.7.11` release contract:** use the signed tag, wheel, source archive,
> SBOM, and release receipt only after they appear on the
> [GitHub release page](https://github.com/weby-homelab/power-framework/releases/tag/v3.7.11).
...
**Profile A — headless / agent server** is complete with one native
`power-framework[mcp]` runtime, `power`, required `power-mcp` over local stdio, the
host-side POWER Skill, and one canonical vault. Docker and Web UI are not
required.

**Profile B — full human + agent server** starts from Profile A and adds exactly
one matching `power-web` container. The Web UI calls the same
`ApplicationService`, mounts the same canonical vault read-write for governed
proposal/apply, stores only rebuildable data in its named cache volume, and is
published on host loopback `127.0.0.1:8080`. The container is non-root and has no
MCP process or MCP TCP port. See [`deploy/web/README.md`](deploy/web/README.md)
for UID/GID provisioning and the authenticated remote-access boundary.
...
Windows 11 25H2 remains outside the `v3.7.11` supported-platform boundary. The
...
made for `v3.7.11`. Windows and macOS have no scheduled release target.

## What's Inside

| Feature                          | What it does                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **CLI**                          | 27 commands, including `power doctor` for read-only runtime/index diagnosis, `power connect` for conflict-safe local MCP setup, `power integrations` for generic suite installers and Skill/config plans, `power cache` for namespace hygiene, `power import` for preflighted Markdown migration, `power memory` for explicit proposal/approval, `power handoff` for compatibility workflows, `power task` for canonical Task v2 lifecycle management, and `power infra` for the constrained local broker                                                                                                                                                                                                                                  |
| **MCP Server**                   | 21 tools, including governed memory context, proposal, apply, validation, history, `handoff_work`, `infra_action`, and search-index synchronization operations                                                                                                                                                                                                                                                                                                                                                    |
...
> **POWER 3.7.11 evidence contract:** publication requires machine validation
...
and MCP inventory are historical evidence, not the current `v3.7.11` contract:
...
Connect P.O.W.E.R. to any MCP-compatible AI client through the local stdio
`power-mcp` launcher. The Web UI is a separate Web-only container and is not an
MCP transport.
The [MCP client onboarding guide](docs/mcp-client-onboarding.md) contains the
...
#### 1. Second Brain (Obsidian Vault) — _Passive Memory Store_
...
#### 2. P.O.W.E.R. Framework — _Active Engine & AI Toolkit_
...
flowchart TD
    ...
    subgraph AI ["🤖 AI Agent (official MCP SDK v2)"]
        Tools[["🔌 21 Async MCP Tools (stdio)"]]:::agent
...
| `core/cli.py`                             | Command-line interface with 27 commands, including read-only doctor diagnostics, generic suite integration plans, conflict-safe MCP connection plans, preflighted import, transactional memory, compatibility handoff workflows, canonical Task v2 lifecycle management, and the constrained `power infra` broker client                                                                              |
| `mcp/power_server.py`                     | Official MCP SDK v2 server with 21 async tools over local stdio                                                                                                                                                                                 |
...
- [Historical P.O.W.E.R. 3.6.3 release notes](docs/release-3.6.3.md) — Canonical Task v2, typed decisions, fail-closed persistence, reranked search, and evidence boundaries.
