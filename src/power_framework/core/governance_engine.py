...
_UNTRUSTED_ACTOR_MARKERS: frozenset[str] = frozenset(
    {"model", "llm", "ai_agent_unverified", "agent:model"}
)


def is_untrusted_actor(actor: str) -> bool:
    """Return whether an actor marker is prohibited from governance authority."""
    return actor.strip().casefold() in _UNTRUSTED_ACTOR_MARKERS
...
def is_untrusted_event(event: ProjectEvent | None) -> bool:
    ...
    if event.source in ("model_extraction", "agent_inference", "untrusted"):
        return True
    if is_untrusted_actor(event.actor):
        return True
