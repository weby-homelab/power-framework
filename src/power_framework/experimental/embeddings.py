...
def dense_embedding_ready() -> tuple[bool, str]:
    ...
    This probe never imports an optional backend, downloads a model, or creates
    a cache directory. It is used by the ``auto`` retrieval profile to keep a
    missing optional install or model snapshot on the FTS path.
    """
    ...
    def module_available(name: str) -> bool:
        try:
            ...
            # A partially installed or deliberately blocked optional package is
            # a degraded capability, not an error in the FTS baseline.
            return False

    if any(not module_available(name) for name in required_modules):
