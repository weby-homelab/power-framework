...
class TestGovernanceActorBoundary:
    @pytest.mark.parametrize("actor", ["model", "llm", "ai_agent_unverified", "agent:model"])
    @pytest.mark.parametrize("evaluation_type", ["dor", "dod"])
    def test_untrusted_actor_cannot_append_governance_evaluation(
        self, tmp_path: Path, actor: str, evaluation_type: Literal["dor", "dod"]
    ) -> None:
        """Untrusted actor markers cannot mint a trusted PSE evaluation event."""
        vault = make_vault(tmp_path)
        pid = "prj_untrusted_eval_actor"
        ...
        with pytest.raises(PermissionError, match="untrusted actor"):
            service.append_governance_evaluation(
                pid,
                evaluation_type,
                actor=actor,
            )
    ...
    def test_non_string_actor_is_rejected_before_ledger_read(self, tmp_path: Path) -> None:
        """Bytes that look like a model marker cannot cross the trusted actor boundary."""
        vault = make_vault(tmp_path)
        pid = "prj_non_string_eval_actor"
        service = _build_closed_project(vault, pid)
        store = ProjectEventStore(pid, vault)
        before_bytes = store.active_events_file.read_bytes()
        before_count = store.verify().event_count
