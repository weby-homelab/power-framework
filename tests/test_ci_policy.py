...
def test_ci_aggregates_all_supported_ubuntu_reports() -> None:
    ...
    assert "name: power372-upgrade-aggregate" in ci_text
    assert "power372-upgrade-aggregate.json" in ci_text
...
def test_release_does_not_require_private_phase8_secrets() -> None:
    ...
    assert "name: power372-stable-release" not in release_text
...
def test_ci_uses_locked_dependencies_and_clean_package_smoke() -> None:
    ...
    assert "power372-gates.json" in release_text
    assert "power372-validation.json" in release_text
    assert "--gate-manifest" in release_text
    assert '--junitxml="$RUNNER_TEMP/power372-junit.xml"' in release_text
    assert '--cov-report=json:"$RUNNER_TEMP/power372-coverage.json"' in release_text
    assert "power372-phase8-technical-receipts" in release_text
    ...
    assert "--upgrade-matrix-aggregate" in release_text
    assert "power372-release-upgrade-aggregate" in release_text
