...
- Restricted privileged `workflow_dispatch` release control to protected `main`.
...
- Recorded the current tag-governance and repository-security boundaries without
  rewriting the published `v3.7.10` release snapshot.
- Classified null CodeQL severity as unknown and kept default-branch alert
  conclusions explicit rather than inferred.
...
• 3.4.0 (Restore Truth): Signed v3.4.0 release, removed false configuration statements.

• 3.4.1 (One Truthful Contract): Single power doctor --json, 20 MCP tools, risk annotations, bilingual documentation.

• 3.4.2 (Closed Mutation Workflow): Atomic write transactions with post-condition checking and rollback scheme.

• 3.4.3 (Durable Handoff): Work packets in Markdown, checkpoints, prompt injection protection.

• 3.4.4 (Fast and Useful Retrieval): Generational caching, matrix vector multiplication, exact dense matrix cache, synthetic quality oracle power31-quality-comparison-v1. Semantic Search (BGE-M3 1024d) is fixed as the only default.

• 3.4.5 (Cross-Platform Release): [DONE] - full coverage of Linux, Windows 11 25H2 (Codex validation) and macOS (CI smoke). Bilingual Support Matrix integrated into CI/drift gates.
...
- Handled structured dictionary targets and section anchors in Second Brain graph builder.
- Refined Low-RAM deployment guidance in English and Ukrainian documentation.
- Decoupled license badge formatting in footer and ensured ruff format compliance.

### Added

- Added P.O.W.E.R. 3.2.0 test evidence report.
