#!/usr/bin/env python3
"""POWER 3.8 Phase 5E R6 — production-faithful evaluation runner.
...
from phase5e_concept_mapping_r6 import (  # noqa: E402
...
def run_benchmark_r6(
    *,
    ...
    if sync_env.status != "ok":
        raise RuntimeError(f"R6 benchmark vault sync failed: {sync_env.degraded_reason}")
    ...
    applicable_outranked_queries = [
        q for q in owner_results if q["shadow"].get("outranked_applicable", False)
    ]
    outranked_count = sum(q["shadow"]["authority_outranked"] for q in applicable_outranked_queries)
    auth_outranked_metric = {
        "applicable_query_count": len(applicable_outranked_queries),
        "measured_violation_count": outranked_count,
        "not_applicable_count": total_query_count - len(applicable_outranked_queries),
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.vault_dir:
        ...
        try:
            vault_dir = Path(tmp_obj.name) / "r6_vault"
            vault_dir.mkdir(parents=True, exist_ok=True)
            results = run_benchmark_r6(
                eval_corpus=eval_corpus,
                split=args.split,
    ...
    if args.output:
        ...
        print(f"R6 results written to {out}")
