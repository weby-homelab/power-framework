#!/usr/bin/env python3
"""POWER 3.8 Phase 5E R4 — production-faithful evaluation runner.
...
from phase5e_concept_mapping import (  # noqa: E402
...
def setup_production_fixtures(vault_dir: Path, manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    # 3. Project fixtures via trusted PSE writer.
    pss = ProjectStateService(vault_dir, task_service=ts, decision_service=ds)
    for concept_id in ("project-current", "contradiction-canonical"):
        concept = concepts.get(concept_id, {})
        payload = concept.get("setup_payload") or {}
        ...
        for ev in events:
            ...
            with contextlib.suppress(Exception):
                # Idempotent setup: skip when the governed head already exists.
                head_exists = False
                try:
...
def run_benchmark_r4(
    *,
    ...
    manifest_sha = _sha256_file(Path(manifest_path))
    ...
    if sync_env.status != "ok":
        raise RuntimeError(f"R4 benchmark vault sync failed: {sync_env.degraded_reason}")
    ...
    with (eval_corpus / f"queries.{split}.jsonl").open("r", encoding="utf-8") as f:
        queries = [json.loads(line) for line in f if line.strip()]
    with (eval_corpus / f"ground_truth.{split}.jsonl").open("r", encoding="utf-8") as f:
        ground_truth = {
    ...
    # Security discriminator: same text as Markdown must never become CANONICAL.
    ...
    for q in queries:
        ...
        expected_concept = (
            source_id_to_concept(str(expected_winner_file), alias_lookup)
            if expected_winner_file
            else None
        )
        # Relevant / exclusion sets mapped to concepts for fair comparison.
        rel_files = set(gt.get("expected_relevant_source_ids", []))
        excl_files = set(gt.get("expected_exclusion_source_ids", []))
        rel_concepts = {source_id_to_concept(str(r), alias_lookup) or str(r) for r in rel_files}
        rel_concepts.discard(None)  # type: ignore[arg-type]
        excl_concepts = {source_id_to_concept(str(r), alias_lookup) or str(r) for r in excl_files}
        excl_concepts.discard(None)  # type: ignore[arg-type]
        graded_rel: dict[str, float] = {}
        for g_item in gt.get("graded_relevance", []):
            mapped = source_id_to_concept(str(g_item["source_id"]), alias_lookup) or str(
                g_item["source_id"]
            )
            graded_rel[mapped] = float(g_item.get("relevance", 1.0))
        ...
        # Legacy execution (concept-mapped for non-regression fairness).
        ...
        leg_excl_leaks = sum(1 for s in leg_concepts[:10] if s in excl_concepts)
        ...
        # Also map full runtime IDs (task:/decision:/project:) directly.
        ...
        query_results.append(
            {
                ...
                "legacy": {
                    ...
                    "mrr": round(leg_mrr, 4),
                    "map": round(leg_map, 4),
                    "ndcg": {k: round(v, 4) for k, v in leg_ndcg.items()},
                    "precision": round(leg_prec, 4),
                    "exclusion_leaks": leg_excl_leaks,
                },
                "shadow": {
                    "latency_ms": round(shad_latency, 2),
                    "concepts": shad_concepts,
                    "raw_ids": shad_raw_ids,
                    "recall": shad_recall,
                    "mrr": round(shad_mrr, 4),
                    "map": round(shad_map, 4),
                    "ndcg": {k: round(v, 4) for k, v in shad_ndcg.items()},
                    ...
                    "total_tokens": shad_total_tokens,
    ...
    shad_summary = {
        ...
        "authority_violations": sum(q["shadow"]["authority_violations"] for q in query_results),
        "authority_winner_missing": sum(
            1 for q in owner_results if not q["shadow"]["winner_present"]
        ),
        "authority_provenance_failures": sum(
            1
            for q in owner_results
            if q["shadow"]["winner_present"] and not q["shadow"]["winner_has_canonical_provenance"]
        ),
        "authority_outranked_by_relevance": sum(
            q["shadow"]["authority_outranked"] for q in owner_results
        ),
    ...
    # Resource bounds (FAST profile, CPU-only): generous but explicit.
    resource_bounds = {
        "p95_latency_ms_limit": 10000.0,
        "mean_tokens_limit": 10000.0,
        "p95_latency_ms_measured": shad_summary["latency_ms_p95"],
        "mean_tokens_measured": _mean([float(q["shadow"]["total_tokens"]) for q in query_results]),
    }
    ...
    # Legacy non-regression (default tolerance: Shadow >= Legacy, no invented margin).
    non_regression = {
    ...
    return {
        "benchmark_metadata": {
            "schema_version": "power.retrieval-benchmark-r4.v1",
            "runner": "benchmark_phase5e_r4",
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--fixture-manifest",
        type=Path,
        default=Path("artifacts/project-state/phase-5e/phase5e_runtime_fixture_manifest_v2.json"),
    )
    ...
    if args.vault_dir:
        ...
        try:
            vault_dir = Path(tmp_obj.name) / "r4_vault"
            vault_dir.mkdir(parents=True, exist_ok=True)
            results = run_benchmark_r4(
                eval_corpus=eval_corpus,
                split=args.split,
