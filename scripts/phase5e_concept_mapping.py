"""POWER 3.8 Phase 5E R4 — evaluation-only concept identity mapping.
...
_MANIFEST_DEFAULT = (
    ...
    / "project-state"
    / "phase-5e"
    / "phase5e_runtime_fixture_manifest_v2.json"
)


def load_manifest(manifest_path: Path | str = _MANIFEST_DEFAULT) -> dict[str, Any]:
    """Load the frozen R4 fixture manifest (read-only)."""
...
def is_owner_backed(
    ...
    """True only for TaskService / DecisionService / ProjectStateService."""
    return owner_for_concept(eval_concept_id, manifest) in {
        "TaskService",
        "DecisionService",
        "ProjectStateService",
    }
