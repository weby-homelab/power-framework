...
def main() -> int:
    ...
    art = args.artifacts
    errors = [f"missing artifact: {name}" for name in sorted(REQUIRED) if not (art / name).is_file()]
    ...
    if summary.get("working_tree_clean_at_start") is not True or summary.get("working_tree_clean_at_end") is not True:
    ...
    if not all(isinstance(summary.get("db_actual", {}).get(key), int) for key in ("doc_embeddings", "chunk_embeddings")):
    ...
    env_text = (art / "environment.txt").read_text(encoding="utf-8", errors="replace") if (art / "environment.txt").exists() else ""
