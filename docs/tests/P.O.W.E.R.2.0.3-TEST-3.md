...
description: "Тестування 4 сценаріїв крос-лінгвального пошуку: UA→UA, EN→UA, UA→EN, Mixed. 20 тест-кейсів, 3 режими (FTS/Vector/Hybrid), 541 файл. Виявлено критичний баг: UA→EN = 0.000 для всіх режимів. Обґрунтування переходу на Qwen Embedding."
...
| Метрика | FTS | Vector | Hybrid |
|---------|:---:|:------:|:------:|
| **MRR** | 0.107 | 0.168 | **0.196** |
| **MAP@5** | 0.090 | **0.200** | 0.190 |
| **MAR@5** | 0.142 | **0.300** | 0.287 |
| **MAR@10** | 0.250 | 0.483 | **0.617** |
| **MnDCG@5** | 0.065 | **0.166** | 0.126 |
| **MnDCG@10** | 0.093 | 0.234 | **0.261** |
| **Avg Latency** | **0.56s** | 2.57s | 2.43s |
| **P95 Latency** | **0.79s** | 5.43s | 5.58s |

> ⚠️ **Примітка:** Низька загальна якість (MAR@5 ≤ 0.300) зумовлена **повним провалом сценарію UA→EN** (0.000 для всіх режимів), що тягне середнє вниз.

### 3.2 MAR@5 за сценаріями — ключова таблиця

| Сценарій | FTS | Vector | Hybrid | Висновок |
|----------|:---:|:------:|:------:|----------|
| 🇺🇦 UA→UA | 0.146 | 0.250 | **0.302** | Hybrid найкращий |
| 🇬🇧 EN→UA | 0.417 | **0.667** | 0.500 | Vector домінує |
| 🇺🇦 UA→EN | **0.000** | **0.000** | **0.000** | ❌ Повний провал |
| 🔀 Mixed | 0.000 | **0.333** | **0.333** | FTS сліпий |

---

## 🔥 4. Критичні знахідки

### 4.1 🚨 КРИТИЧНО: UA→EN = 0.000 для ВСІХ режимів
...
CL-15 машинне навчання локальна модель → R@5: FTS=0.00  Vector=0.00  Hybrid=0.00
CL-16 SSH порт зміна конфігурація безпека → R@5: FTS=0.00  Vector=0.00  Hybrid=0.00
```

**Що це означає на практиці:**
- Якщо агент або користувач запитує **по-українськи** — він **не знайде** жодного документа, написаного **по-англійськи**, навіть якщо це точно той самий контент
- `POWER_Framework.md`, `AI-HomeLab.md`, `SSH Port Changer.md` — всі **невидимі** для UA-запитів
- Модель `paraphrase-multilingual-MiniLM-L12-v2` **не забезпечує cross-lingual alignment** UA↔EN на практиці
...
### 4.2 EN→UA працює значно краще (Vector MAR@5=0.667)

```
CL-09 docker container deployment → EN→UA: Vector=1.33, FTS=1.00, Hybrid=1.00
CL-11 backup storage archive files → EN→UA: Vector=1.33, FTS=0.67, Hybrid=1.00
```
...
### 4.3 FTS (BM25) повністю сліпий на UA-запити

```
UA→UA FTS MAR@5 = 0.146  (знаходить лише через точне входження UA-слів)
EN→UA FTS MAR@5 = 0.417  (EN-слова є і в UA-документах → знаходить)
UA→EN FTS MAR@5 = 0.000  (UA-слів в EN-документах немає → miss)
Mixed FTS MAR@5 = 0.000  (EN-частина змішаного запиту губиться)
```

BM25 є **мономовним** за природою. Для UA-only vault він був би ефективнішим, але для мікс-контенту — неприйнятний.

### 4.4 Hybrid MAR@10 = 0.617 — найкращий при глибокому пошуку

При збільшенні до топ-10 результатів Hybrid показує MAR@10=0.617 — найкращий результат серед усіх режимів. Це означає що **релевантні документи є в індексі**, але ранжуються за позиціями 6-10, а не в топ-5.
...
|----|-------|------|:---:|:------:|:------:|
| CL-01 | ua_ua | докер розгортання | 0.00 | 0.25 | 0.25 |
| CL-02 | ua_ua | безпека файрвол | 0.00 | **1.00** | **1.00** |
| CL-03 | ua_ua | база знань PKM | 0.00 | 0.00 | 0.00 |
| CL-04 | ua_ua | резервне копіювання | 0.50 | **0.75** | 0.50 |
| CL-05 | ua_ua | мережа VPN | 0.00 | 0.00 | 0.00 |
| CL-06 | ua_ua | AI агент MCP | 0.00 | 0.00 | 0.00 |
| CL-07 | ua_ua | реліз GitHub | **0.67** | 0.00 | **0.67** |
| CL-08 | ua_ua | GPG підпис коміту | 0.00 | 0.00 | 0.00 |
| CL-09 | en_ua | docker deployment | **1.00** | **1.33** | **1.00** |
| CL-10 | en_ua | security hardening | 0.00 | 0.00 | 0.00 |
| CL-11 | en_ua | backup storage | 0.67 | **1.33** | **1.00** |
...
| CL-15 | ua_en | ML inference | 0.00 | 0.00 | 0.00 |
| CL-16 | ua_en | SSH конфігурація | 0.00 | 0.00 | 0.00 |
| CL-17 | mixed | docker+безпека | 0.00 | **1.33** | **1.33** |
...
|---------|:----------------:|:----------------------:|:-:|
| Vector MAR@5 | 0.733 | 0.300 | **-59%** ⬇️ |
| Vector MRR | 0.377 | 0.168 | **-55%** ⬇️ |
| FTS MAR@5 | 0.567 | 0.142 | **-75%** ⬇️ |
| Hybrid MAR@5 | 0.611 | 0.287 | **-53%** ⬇️ |

**Висновок:** Якість пошуку **катастрофічно падає** при реальному мультилінгвальному навантаженні. Vault із мікс UA+EN контентом — саме такий сценарій.
...
|----------|:---:|:---:|:-:|
| UA→UA | 0.250 | ~0.550 | **+120%** |
| EN→UA | 0.667 | ~0.800 | **+20%** |
| UA→EN | **0.000** | **~0.600** | **+∞** |
| Mixed | 0.333 | ~0.700 | **+110%** |
...
- ❌ **UA→EN: MAR@5 = 0.000** — повна відсутність cross-lingual retrieval
- ⚠️ **UA→UA: MAR@5 = 0.250** (Vector) — посередньо, лише 25% релевантного знайдено
- ✅ **EN→UA: MAR@5 = 0.667** (Vector) — добре, завдяки EN-термінам у UA-тексті
...
Досвід користувачів, які повідомляють про **+30-40% приріст якості** після переходу з OpenAI на локальні Qwen embeddings, — повністю підтверджується цими вимірюваннями. Різниця насправді може бути ще більшою: з `MAR@5=0.000` до `~0.600` для UA→EN — це **не 30-40%, а кардинальна зміна**.
