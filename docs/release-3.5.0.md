...
- The executable upgrade matrix also kills a subprocess at `before_move`,
  ...
  hermetic synthetic fault matrix, not physical 3.4.5 runtime or Windows/macOS
...
- Crash recovery reaps only lease-expired `building` rows and their UUID-owned
  ...
  uploads one content-free report per supported OS and blocks on their aggregate.

## Evidence boundary

The candidate gate records local tests, lint, types, documentation drift, the
FTS-first packaging contract, and the aggregated executable upgrade matrix.
Dense quality, real-vault outcome improvement, blind sealed-human scoring,
physical Windows/macOS migration, signed tag, package indexes, and remote
...
CI executes the content-free upgrade invariants on Linux, macOS, and Windows;
the local runner report intentionally records only the platform actually
executed. Final publication additionally requires a clean tag-bound baseline generated by
