...
This is the canonical local-stdio setup for P.O.W.E.R. `v3.6.2` on Linux.
It gives Codex, OpenCode, Gemini CLI, Claude Desktop, and Claude Code
the same server process and the same vault boundary.

> **Release contract:** use `v3.6.2` only after its signed tag and immutable
> wheel appear on the [GitHub release page](https://github.com/weby-homelab/power-framework/releases/tag/v3.6.2).
