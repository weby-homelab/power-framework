...
Це авторитетний clean-install шлях для P.O.W.E.R. `v3.6.3`. Він створює лише
новий vault. Для наявних нотаток використовуйте
[гід міграції](migration-guide.ua.md), а не запускайте `power init` поверх них.

> **Контракт релізу:** використовуйте `v3.6.3` лише після появи signed tag та
> immutable wheel на [сторінці релізу GitHub](https://github.com/weby-homelab/power-framework/releases/tag/v3.6.3).
...
Обидві команди версії мають показати `3.6.3`, а остання команда —
`lean FTS import: OK`.
...
- CLI та distribution metadata повертають `3.6.3`.
