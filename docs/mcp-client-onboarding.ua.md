...
Це канонічне налаштування локального stdio для P.O.W.E.R. `v3.7.7` на Linux.
Воно дає Codex, OpenCode, Gemini CLI, Claude Desktop і Claude Code один
і той самий процес сервера та однакову межу vault.

> **Контракт релізу:** використовуйте `v3.7.7` лише після появи signed tag та
> immutable wheel на [сторінці релізу GitHub](https://github.com/weby-homelab/power-framework/releases/tag/v3.7.7).
...
підтримуваними MCP onboarding платформами для `v3.7.7`.
...
"$POWER_PYTHON" -m pip install \
  "power-framework[mcp] @ https://github.com/weby-homelab/power-framework/releases/download/v3.7.7/power_framework-3.7.7-py3-none-any.whl"
...
Встановлений public launcher — `~/.local/bin/power-mcp`. Реалізація міститься в
пакеті `power_framework.mcp`, але клієнти повинні використовувати цей єдиний
managed stdio entry point; ad-hoc Python module і Docker MCP runner не входять до
release contract.
