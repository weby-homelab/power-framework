...
Це канонічне налаштування локального stdio для P.O.W.E.R. `v3.7.8` на Linux.
Воно дає Codex, OpenCode, Gemini CLI, Claude Desktop і Claude Code один
і той самий процес сервера та однакову межу vault.

> **Контракт релізу:** використовуйте `v3.7.8` лише після появи signed tag та
> immutable wheel на [сторінці релізу GitHub](https://github.com/weby-homelab/power-framework/releases/tag/v3.7.8).
...
підтримуваними MCP onboarding платформами для `v3.7.8`.
