---
type: Resource
title: "AI Agent Migration Guide: Any Markdown Knowledge Base to P.O.W.E.R. v3.7.11"
...
# AI Agent Migration Guide: Any Markdown Knowledge Base to P.O.W.E.R. v3.7.11
...
This guide targets the `v3.7.11` release. Select the clean-install
...
- The destination uses canonical P.O.W.E.R. top-level folders so hierarchical
  indexing and the 21 MCP tools have their documented behavior.
...
This guide is verified against the `v3.7.11` release contract. CI checks these
facts against the executable capability manifest so an agent does not inherit
an older migration recipe:

- the current surface is 27 top-level CLI commands and 21 MCP tools;
...
Use the `v3.7.11` environment from Getting Started and verify:
