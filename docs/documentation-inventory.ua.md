...
| --- | --- | --- |
| `README.md` | Англійський огляд і маршрути для агента | Вирівняно з 3.7.8 release |
...
| `docs/getting-started.ua.md` | Українська чиста установка | Семантично паритетна з 3.7.8 release |
...
| Release wheel `v3.7.8` | Unified install artifact | Tag-bound wheel; signed tag/readback перевіряє GitHub Release workflow |
...
- Видалено небезпечне оновлення через destructive reset, непереносний `/tmp`,
  ...
  інтерпретатора venv.
- Встановлення прив'язано до release wheel `v3.7.8`, а не до
...
перевірки](tests/windows-11-25h2-validation.md). `v3.7.8` не запускає
