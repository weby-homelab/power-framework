...
| --- | --- | --- |
| `README.md` | Англійський огляд і маршрути для агента | Вирівняно з 3.7.4 candidate |
...
| `docs/getting-started.ua.md` | Українська чиста установка | Семантично паритетна з 3.7.4 candidate |
...
| Release wheel `v3.7.4` | Candidate install artifact | Tag-bound candidate wheel; signed tag/readback перевіряє GitHub Release workflow |
...
- Видалено небезпечне оновлення через destructive reset, непереносний `/tmp`,
  ...
  інтерпретатора venv.
- Встановлення прив'язано до candidate release wheel `v3.7.4`, а не до
...
перевірки](tests/windows-11-25h2-validation.md). `v3.7.4` не запускає
