...
| --- | --- | --- |
| `README.md` | Англійський огляд і маршрути для агента | Вирівняно з 3.7.11 release |
...
| `docs/getting-started.ua.md` | Українська чиста установка | Семантично паритетна з 3.7.11 release |
...
| Release wheel `v3.7.11` | Unified install artifact | Tag-bound wheel; signed tag/readback перевірятиме GitHub Release workflow |
...
- Видалено небезпечне оновлення через destructive reset, непереносний `/tmp`,
  ...
  інтерпретатора venv.
- Встановлення прив'язано до release wheel `v3.7.11`, а не до
...
перевірки](tests/windows-11-25h2-validation.md). `v3.7.11` не запускає
