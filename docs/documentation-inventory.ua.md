...
**Версія коду:** 3.7.9 release candidate
...
| --- | --- | --- |
| `README.md` | Англійський огляд і маршрути для агента | Вирівняно з 3.7.9 release candidate |
...
| `docs/getting-started.ua.md` | Українська чиста установка | Семантично паритетна з 3.7.9 release candidate |
...
| Release wheel `v3.7.9` | Unified install artifact | Candidate tag-bound wheel; signed tag/readback перевірятиме GitHub Release workflow |
...
- Видалено небезпечне оновлення через destructive reset, непереносний `/tmp`,
  ...
  інтерпретатора venv.
- Встановлення прив'язано до candidate release wheel `v3.7.9`, а не до
...
перевірки](tests/windows-11-25h2-validation.md). `v3.7.9` не запускає
