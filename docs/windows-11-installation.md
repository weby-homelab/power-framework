...
> **Not a `v3.7.2` procedure.** Windows support is deferred indefinitely and
> has no scheduled release target. The commands below preserve the historical
> `v3.5.0` host-validation procedure and must not be presented as 3.7.2
> compatibility or release certification for the 3.7.2 candidate.
