# Informational: install P.O.W.E.R. 3.7.8 on Windows 11 25H2

> **Not a supported `v3.7.8` platform.** Windows support is deferred
...
> certification for the 3.7.8 Linux release boundary.

This guide installs P.O.W.E.R. `v3.7.8` in an isolated virtual environment,
creates a clean vault, verifies the CLI, and configures an MCP client. It uses
PowerShell syntax throughout.

> **Release artifact:** after publication, use only the signed `v3.7.8` tag and
> immutable wheel from the [GitHub release page](https://github.com/weby-homelab/power-framework/releases/tag/v3.7.8).
...
- P.O.W.E.R. `v3.7.8` includes an automated cross-platform regression for the
...
- Physical Windows 11 25H2 validation was completed on 2026-08-08 for follow-up
  ...
  immutable `v3.7.8` release artifacts.
...
```powershell
$ReleaseWheel = "https://github.com/weby-homelab/power-framework/releases/download/v3.7.8/power_framework-3.7.8-py3-none-any.whl"
...
Both version checks must report `3.7.8`, and the import check must print
...
```powershell
& $VenvPython -m pip install "git+https://github.com/weby-homelab/power-framework.git@v3.7.8"
```
