...
Starting with the 3.7.7 release, the Web UI is physically integrated into the
...
6. **Web UI Presentation Adapter:**
    - Source resides in `src/power_framework/web/`
     - Production server profile runs as an OCI container (`ghcr.io/weby-homelab/power-framework-web:3.7.7`)
...
`power-web` container from the matching `3.7.7` release image. The canonical
