...
6. **Web UI Presentation Adapter:**
    - Source resides in `src/power_framework/web/`
      - Production server profile runs as an OCI container (`ghcr.io/weby-homelab/power-framework-web:3.7.11`)
...
`power-web` container from the matching `3.7.11` release image. The canonical
