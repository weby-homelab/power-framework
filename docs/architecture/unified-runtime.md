...
the current patch runtime boundary is 3.7.10.
...
6. **Web UI Presentation Adapter:**
    - Source resides in `src/power_framework/web/`
      - Production server profile runs as an OCI container (`ghcr.io/weby-homelab/power-framework-web:3.7.10`)
...
`power-web` container from the matching `3.7.10` release image. The canonical
