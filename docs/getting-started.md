...
This is the authoritative clean-install path for P.O.W.E.R. `v3.6.5`. It
...
> **Candidate release contract:** use `v3.6.5` only after its signed tag and immutable
> wheel appear on the [GitHub release page](https://github.com/weby-homelab/power-framework/releases/tag/v3.6.5).
...
Both version commands must report `3.6.5`; the final command must print
`lean FTS import: OK`.
...
```bash
"$POWER_PYTHON" -m pip install \
  "power-framework[remote] @ https://github.com/weby-homelab/power-framework/releases/download/v3.6.5/power_framework-3.6.5-py3-none-any.whl"
```
...
```bash
"$POWER_PYTHON" -m pip install \
  'git+https://github.com/weby-homelab/power-framework.git@v3.6.5'
```
...
- CLI and distribution metadata both report `3.6.5`.
