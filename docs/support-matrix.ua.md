...
Ця матриця визначає операційні межі контракту кандидата релізу `v3.7.2`. Вона
...
| macOS | Відкладено на невизначений строк для `v3.7.2`; macOS CI та release upgrade evidence не заявляються. | Цільового релізу не заплановано. Майбутня пропозиція потребує названого runner, owner і свіжих receipts. | Усі claims сумісності, продуктивності, GPU, dense real-vault і reranker для macOS. |
| Hosted Windows CI | Відкладено на невизначений строк для `v3.7.2`; Windows CI та release upgrade evidence не заявляються. | Цільового релізу не заплановано. Майбутня пропозиція потребує названого runner, owner і свіжих receipts. | Усі claims lifecycle, provider, performance, GPU, dense real-vault і quality для Windows. |
| Фізичний Windows 11 25H2 | Відкладено на невизначений строк для `v3.7.2`; installation guide є лише інформаційним і не є release certification. | Цільового релізу не заплановано. Майбутня пропозиція потребує точних host/artifact receipts. | Фізична сумісність Windows, GPU performance, CUDA availability і latency claims. |
...
але Windows і macOS не входять до межі `v3.7.2` та не мають запланованого
release target.
