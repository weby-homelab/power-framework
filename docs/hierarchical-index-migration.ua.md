...
**Дата:** 03 липня 2026
**Версія:** P.O.W.E.R. v1.6.0
**Автор:** Weby Homelab AI Team
**Статус:** Історичний зріз v1.6; не є чинним контрактом встановлення чи міграції

!!! warning "Історичні дані, а не поточна гарантія"
    Звіт зберігає вимірювання однієї бази з 324 нотаток і стан реалізації v1.6. Він
    не доводить універсальної токен-складності чи поточної production-продуктивності.
    Для актуальних дій використовуйте [чисте встановлення](getting-started.ua.md),
    [міграцію наявної бази](migration-guide.ua.md), а також чинні довідники
    [CLI](cli.md) і [MCP](mcp-server.md).
...
**Історичне спостереження:** вибіркове читання зменшило кількість токенів у цій
виміряній базі. Згенеровані каталоги все одно ростуть із кількістю нотаток, тому
звіт не заявляє асимптотики `O(log n)`.
...
2. **PEP 668 (Externally-Managed Environments):** системний Python може
   відхилити пряме встановлення через `pip`. Чинна рекомендація — створити
   окремий venv і вказати кожному MCP-клієнту точний шлях до його інтерпретатора.

3. **Git rebase conflicts:** початкова міграція мала розбіжну історію. Чинна
   рекомендація — перевірити refs, зберегти локальну роботу й використати окрему
   гілку/PR; руйнівний reset або force-push не є типовим кроком міграції.
...
2. **Вибіркова навігація** — агент читає потрібний підіндекс, а не всю базу
