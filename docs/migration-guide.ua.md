...
### Крок 6j: Багатомодовий пошук (FTS + Vector + Hybrid)

Фреймворк P.O.W.E.R. включає вбудовану систему багатомодового пошуку з трьома стратегіями:
...
| `vector` | TF-векторна косинусна схожість (pure Python, zero deps) | Семантичний пошук за змістом |
| `hybrid` | RRF (Reciprocal Rank Fusion) злиття FTS + Vector | Найкраще покриття результатів |

*Правила використання пошуку для AI-агентів:*
1. **Економія токенів:** Використовуйте `search_vault_tool(query, max_results=20, search_mode="hybrid")` замість `glob **/*.md`. Це знижує споживання контексту на 95%+.
2. **Вибір режиму:**
   - **FTS** — для точних запитів: `search_vault_tool(query='"Docker Compose"')`
   - **Vector** — для концептуальних пошуків: `search_vault_tool(query="оркестрація деплою", search_mode="vector")`
   - **Hybrid** — для загального пошуку: `search_vault_tool(query="налаштування сервера", search_mode="hybrid")`
3. **CLI використання:** `power search /vault "запит" --mode hybrid`
4. **Синтаксис запитів:**
