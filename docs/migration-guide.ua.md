---
type: Resource
title: "Ґайд міграції для AI-агента: як перенести будь-який Obsidian Vault у структуру P.O.W.E.R. (v2.0.1)"
description: "Покроковий протокол для будь-якого LLM-агента для автономної міграції Obsidian Vault у OKF-сумісну структуру P.O.W.E.R. під версію v2.0.1."
tags: [power, migration, guide, ai-agents, mcp, bge-m3, graphrag]
timestamp: 2026-07-15T02:00:00
---

# Ґайд міграції для AI-агента: як перенести будь-який Obsidian Vault у структуру P.O.W.E.R. (v2.0.1)
...
### Крок 6a: Встановлення та Налаштування P.O.W.E.R. Framework (v2.0.1)

Для автономної роботи агента на цільовому хості встановіть інструментарій P.O.W.E.R. (v2.0.1) глобально або у віртуальне середовище проєкту:

```bash
pip install git+https://github.com/weby-homelab/power-framework.git
```

#### 🧠 Двомовне Налаштування та ONNX (Оновлення v2.0.1)
Починаючи з версії 2.0.1, модель вбудовування за замовчуванням оновлена до **`BAAI/bge-m3`** (розмірність векторів **1024**) для повноцінної двомовної (EN + UA) підтримки семантичного пошуку.

Щоб уникновувати помилок виходу директорії ONNX Runtime (`External data path escapes model directory`) під час ініціалізації моделі на CPU, у фреймворку реалізовано:
1. **Вимкнення символьних посилань**: Агент або середовище повинні встановити змінну:
   ```bash
   export HF_HUB_DISABLE_SYMLINKS=1
   ```
   Це змушує Hugging Face завантажувати реальні файли ваг замість символічних посилань.
2. **Реєстрація моделі**: Фреймворк автоматично реєструє модель через `TextEmbedding.add_custom_model` з джерелом `onnx-community/bge-m3-ONNX`, додаючи обов'язковий файл `onnx/model.onnx_data` до списку додаткових файлів.
...
Починаючи з v2.0.0, ви можете налаштувати LLM-сервери (`POWER_LLM_*`) для автоматичного аудиту та оновлення метаданих. Застосуйте параметр `"opencode"` для прямої роботи через локальний CLI OpenCode без додаткових інференс-серверів:
...
Фреймворк P.O.W.E.R. (v2.0.1) включає вбудовану пошукову систему, яка підтримує п'ять стратегій:
...
| `semantic` | Косинусна схожість густих векторів (`BAAI/bge-m3` на CPU) | Двомовний (EN+UA) семантичний пошук |
...
| `_index.md` не має frontmatter | Використовується стара версія фреймворку | Оновіться до v2.0.1 або перезапустіть `generate_index` |
