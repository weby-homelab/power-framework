...
- **Status:** APPROVED & COMPLETE (Closure Correction Round 3 Applied)
...
The implementation satisfies all Phase 2 architectural requirements and strictly adheres to frozen Phase 1 ADRs (ADR-PSE-001 through ADR-PSE-008) and Phase 2 closure corrections (Rounds 1, 2, and 3).
...
2. **`power_framework.core.lock_tracker`**: Strict 3-level lock acquisition hierarchy (Level 1 Mutation -> Level 2 Task -> Level 3 Project). Hardened with Level 3 `project_id` tracking, forbidding cross-project nested lock acquisitions (`LockHierarchyViolationError`) to prevent deadlocks while supporting intra-thread reentrancy for the same project.
3. **`power_framework.core.project_models`**: Strict Pydantic v2 schemas (`extra="forbid"`) for `ProjectEvent`, `AppendCommand`, `PrivacyMode`, `RedactionRecord`, exactly 44 event types, deterministic regex-safe event ID generation (`generate_deterministic_event_id`), and mandatory payload contract validation & normalization for Cross-Subsystem Association Sagas (`TaskAssociationRequestedPayload`, etc.) across both commands and canonical events.
...
5. **`power_framework.core.project_ingestion`**: Safe application-level ingestion boundary with multi-tier privacy modes (`metadata-only`, `structured-events`, `full-content`), defense-in-depth secret scrubbing, total elimination of raw dialogue/transcripts from event payloads across ALL privacy modes (stored strictly in `.power/raw-evidence/<project_id>/<event_id>.json` with mode `0600`, pre-validation before write guaranteeing zero orphan evidence files, and `O_EXCL` atomicity), fail-closed batch import (`import_project_events`) with pre-import ledger verification and the exact atomicity guarantee: *"complete batch pre-validation with zero canonical ledger mutation on validation/contract failure, followed by ordered durable append and fsync; torn-tail recovery protects interrupted physical writes"*, pre-verified fail-closed disposable derived SQLite indexing (with clear boundary & TOCTOU delineation between canonical ledger and disposable projection), Diagnostic Ledger Summary markdown generation, and true crash/restart-safe cross-subsystem saga reconciliation appending each retry attempt to the canonical ledger.
...
| **G2.2** | **Duplicate Delivery Idempotency & Conflict Detection** | **PASSED** | `tests/test_phase2_event_ledger.py::test_duplicate_delivery_idempotency`<br>`tests/test_phase2_event_ledger.py::test_idempotency_conflict_raises_error`<br>`tests/test_phase2_event_ledger.py::test_evidence_retry_with_changed_content_raises_idempotency_conflict`<br>`tests/test_phase2_event_ledger.py::test_deterministic_event_id_generation_and_regex_safety`<br>Repeated append calls with identical command fingerprints return existing events idempotently. Appends with matching `idempotency_key` but conflicting payload or raw evidence content raise `IdempotencyConflictError`. Event IDs derived from idempotency keys use deterministic SHA-256 digests (`^evt_[a-z0-9_-]{8,64}$`), safely supporting complex Unicode, punctuation, and 128-char keys. |
...
| **G2.7** | **Privacy Boundary & Raw Content Prohibition** | **PASSED** | `tests/test_phase2_event_ledger.py::test_privacy_modes_verification`<br>`tests/test_phase2_event_ledger.py::test_raw_dialogue_prohibited_across_all_privacy_modes`<br>`tests/test_phase2_event_ledger.py::test_secret_redaction_pipeline_fixtures`<br>`tests/test_phase2_event_ledger.py::test_import_rejects_raw_dialogue_payload`<br>`tests/test_phase2_event_ledger.py::test_import_rejects_invalid_saga_payload`<br>`tests/test_phase2_event_ledger.py::test_evidence_atomic_0600_permissions_and_durability`<br>`tests/test_phase2_event_ledger.py::test_metadata_only_saga_preserves_structural_contracts`<br>`tests/test_phase2_event_ledger.py::test_reconciliation_retry_state_survives_process_restart`<br>`tests/test_phase2_event_ledger.py::test_reconciliation_decision_retry_state_survives_process_restart`<br>`tests/test_phase2_event_ledger.py::test_command_validation_failure_leaves_zero_orphan_raw_evidence`<br>Raw dialogue and LLM transcript keys are prohibited from ledger payloads across all privacy modes and during batch imports. In `full-content` mode, raw dialogue is stored atomically in `.power/raw-evidence/<project_id>/<event_id>.json` (`0600` mode, `O_EXCL`, `fsync`) ONLY after all command and payload validations succeed, guaranteeing zero orphan files. Saga events preserve required structural contracts in `metadata-only` mode, and retries are recorded as new requested events in the canonical ledger. |

---

## 3. Closure Corrections (Rounds 2 and 3) — Defects Remediated

All fundamental ledger defect categories identified in Rounds 2 and 3 have been comprehensively resolved:
1. **`import_project_events()` Exact Atomicity Contract & Fail-Closed Pre-validation:**
   - Strictly guarantees: *complete batch pre-validation with zero canonical ledger mutation on validation/contract failure, followed by ordered durable append and fsync; torn-tail recovery protects interrupted physical writes.*
   - Does not claim physical crash-atomic multi-record transaction.
   - Calls `store.verify()` before writing anything; raises `LedgerIntegrityError` if existing ledger is corrupted with zero bytes changed.
   ...
   - Performs ordered durable append under Level 3 lock with `os.fsync`.
2. **Normalization and Canonization of Saga Payloads:**
   - In `AppendCommand` and `ProjectEvent`, saga payloads are validated and stored as normalized Pydantic model dumps.
   - In `metadata-only` privacy mode, saga structural fields are preserved to prevent contract breakage.
3. **Deterministic Regex-Safe Event ID Generation & Zero Orphan Raw Evidence:**
   - Generates deterministic, regex-safe `event_id` from crypto digest `f"evt_{hashlib.sha256(f'{project_id}:{idempotency_key}'.encode()).hexdigest()[:24]}"`, safely accommodating `:`, `/`, spaces, Unicode, and 128-char keys while strictly matching `^evt_[a-z0-9_-]{8,64}$` and `EVENT_ID_REGEX`.
   - Enforces strict pre-validation of all command and payload contracts BEFORE creating raw evidence files, guaranteeing zero orphan files on validation failures.
   - Evidence files written using `os.O_WRONLY | os.O_CREAT | os.O_EXCL | os.O_NOFOLLOW` with mode `0600` and `os.fsync`.
...
5. **True Crash/Restart-Safe Saga Reconciliation:**
   - Each retry attempt is recorded directly in the canonical ledger as a new `task.association.requested` / `decision.association.requested` event with `attempt = past_attempts + 1`, deterministic idempotency key `f"reconcile:{corr_id}:attempt:{attempt}"`, and remains pending while `attempt < max_attempts`.
   - When `attempt >= max_attempts`, appends `*.association.failed` event to canonical ledger.
   - Reconstructs past attempts from canonical ledger history (`store.replay()`), ensuring retry counts survive complete in-memory process crashes and restarts without manual interventions.
   - Composite key `(project_id, association_kind, entity_id, correlation_id)` guarantees isolation across projects and entities.
...
7. **Comprehensive Symlink & Boundary Defense (TOCTOU Delineation):**
   - Canonical event ledgers and raw evidence files strictly enforce race-free `O_NOFOLLOW` atomic opens and boundary verification.
   - Secondary SQLite database is a disposable derived projection and is protected by path resolution and symlink checks without kernel-level `O_NOFOLLOW` TOCTOU guarantees at the SQLite core layer.
8. **16 Comprehensive Regression Tests:**
   - 16 mandated regression tests added to `tests/test_phase2_event_ledger.py` (including restart safety across multiple wipes, decision saga restart safety, complex deterministic event IDs, and zero orphan evidence verification) passing 100%.

---

## 4. Test Suite & Verification Metrics

- **Targeted Phase 2 Tests:** 51 passed in 1.01s (`tests/test_phase2_event_ledger.py`, 35 original + 16 new regression tests)
- **Full Framework Test Suite:**
  ...
  - Result: **1,478 passed, 4 skipped, 17 deselected in 93.03s**
  - Code Coverage: **82.52%** (exceeds mandatory 70% threshold)
...
> All test suites, contract validations, static analysis, and regression checks have been executed and verified locally on workstation `ws` with 100% success. Upstream GitHub CI workflows (`ci.yml`, `codeql.yml`, `docs.yml`, `release.yml`) are configured to trigger on `push` to `main`, `pull_request` to `main`, or `workflow_dispatch`. Pushing directly to feature branch `feat/power-3.8-project-state-engine` therefore does not trigger CI runs by design; opening a Pull Request targeting `main` activates the automated GitHub Actions CI workflow against the exact commit SHA.
