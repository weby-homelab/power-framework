...
2. **Authoritative Canonical JSON & Server-Side Hashing:**
   ...
   - POWER Canonical JSON v1 is the authoritative serialization contract for ProjectEvent v1.
     ...
     ```
   - Encoding: UTF-8 without BOM, keys sorted lexicographically at all nesting levels (`sort_keys=True`), compact separators `(',', ':')`, non-ASCII preserved without escaping (`ensure_ascii=False`).
...
Event 1 (Genesis, seq=1):
  ...
  event_hash = SHA256(POWER_CANONICAL_JSON_V1(integrity_record_1))

Event 2 (seq=2):
  prev_event_hash = Event 1.event_hash
  payload_digest = SHA256(POWER_CANONICAL_JSON_V1(payload))
  event_hash = SHA256(POWER_CANONICAL_JSON_V1(integrity_record_2))
```
...
- **Two-Tier Verification:**
  1. `payload_digest == SHA256(POWER_CANONICAL_JSON_V1(payload))` verifies inner payload authenticity.
  2. `event_hash == SHA256(POWER_CANONICAL_JSON_V1(integrity_record))` verifies envelope immutability.
...
1. **Three Explicit Privacy Modes:**
   ...
   - `full-content` (Explicit Opt-In Only): sanitized conversation turns retained in a local-only raw-evidence store (`.power/raw-evidence/` with mode `0600`/`0700`). Retention period (TTL) is 14 days by default, is configurable by the user/administrator (0 = disabled), and is NOT an immutable format invariant. NEVER stored in event payloads, NEVER committed to Git, and NEVER synced across fleet nodes by default.
